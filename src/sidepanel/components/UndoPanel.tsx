...
import type { BulkActionSubItem } from '../../shared/undoTypes';
...
const UndoPanel: React.FC<UndoPanelProps> = ({ targetTabId, onUndoComplete }) => {
  ...
  const handleUndo = async (action: UndoAction) => {
    ...
    const isBulkAction =
      action.type.startsWith('BULK_') || action.type === 'CONVERT_USER_TO_GROUP_ASSIGNMENT';
  ...
  const handleClearHistory = async () => {
    if (
      !confirm('Are you sure you want to clear the entire undo history? This cannot be undone.')
    ) {
      return;
    }
  ...
  const getActionTypeLabel = (action: UndoAction): string => {
    ...
    switch (metadata.type) {
      ...
      case 'BULK_REMOVE_USERS_FROM_GROUP':
        return hasPartial
          ? `Bulk Removal (${remainingCount}/${totalCount})`
          : `Bulk Removal (${totalCount})`;
      case 'BULK_ADD_USERS_TO_GROUP':
        return hasPartial
          ? `Bulk Addition (${remainingCount}/${totalCount})`
          : `Bulk Addition (${totalCount})`;
      case 'ACTIVATE_RULE':
      ...
      case 'BULK_ACTIVATE_RULES':
        return hasPartial
          ? `Bulk Activation (${remainingCount}/${totalCount})`
          : `Bulk Activation (${totalCount})`;
      case 'BULK_DEACTIVATE_RULES':
        return hasPartial
          ? `Bulk Deactivation (${remainingCount}/${totalCount})`
          : `Bulk Deactivation (${totalCount})`;
      case 'REMOVE_USER_FROM_APP':
      ...
      case 'BULK_REMOVE_USERS_FROM_APP':
        return hasPartial
          ? `App Bulk Removal (${remainingCount}/${totalCount})`
          : `App Bulk Removal (${totalCount})`;
      case 'BULK_REMOVE_GROUPS_FROM_APP':
        return hasPartial
          ? `App Group Removal (${remainingCount}/${totalCount})`
          : `App Group Removal (${totalCount})`;
      case 'CONVERT_USER_TO_GROUP_ASSIGNMENT':
        return hasPartial
          ? `Assignment Conversion (${remainingCount}/${totalCount})`
          : `Assignment Conversion (${totalCount})`;
      default:
        return 'Unknown';
    }
  };
  ...
  const renderBulkActionSubItems = (action: UndoAction) => {
      ...
      return (
        <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
          <p className="text-sm text-green-800 font-medium">
            ✓ All items in this bulk action have been undone
          </p>
        </div>
      );
    }
    ...
    if (
      ...
      metadata.type === 'BULK_ACTIVATE_RULES' ||
      metadata.type === 'BULK_DEACTIVATE_RULES'
    ) {
      items = action.subItems.map((subItem) => {
    ...
    return (
      <details className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <summary className="cursor-pointer text-sm font-medium text-gray-700 hover:text-gray-900 flex items-center justify-between">
          <span>
            View all {action.subItems.length} items ({remainingItems.length} remaining)
          </span>
        ...
        <div className="mt-3 space-y-1 max-h-64 overflow-y-auto">
          {items.map((item, index) => (
            ...
            >
              ...
              <span
                className={`flex-1 ${item.status === 'completed' ? 'font-medium text-gray-900' : 'text-gray-600 line-through'}`}
              >
                {item.name}
              </span>
              {item.status === 'undone' && (
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2 text-sm">
              ...
              <span className="text-gray-900">
                {metadata.userName} ({metadata.userEmail})
              </span>
            </div>
            <div className="flex items-start gap-2 text-sm">
          ...
          <div className="space-y-2">
            ...
            {metadata.type === 'BULK_REMOVE_USERS_FROM_GROUP' && metadata.operationType && (
              <div className="flex items-start gap-2 text-sm">
                ...
                <span className="text-gray-900">
                  {metadata.operationType === 'deprovisioned'
                    ? 'Remove Deprovisioned'
                    : metadata.operationType === 'inactive'
                      ? 'Smart Cleanup (Inactive)'
        ...
        return (
          <div className="space-y-2">
            ...
            <div className="flex items-start gap-2 text-sm">
              ...
              <span
                className={
                  metadata.userAssignmentsRemoved ? 'text-green-700 font-semibold' : 'text-gray-600'
                }
              >
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <Button variant="secondary" size="sm" onClick={handleClearHistory} disabled={isLoading}>
          Clear History
        </Button>
      </div>

      {error && <AlertMessage message={{ text: error, type: 'error' }} />}

      <div className="space-y-3">
        {undoableActions.map((action) => {
          const remainingCount = getRemainingCount(action);
          const hasPartiallyCompleted =
            action.subItems && remainingCount < action.subItems.length && remainingCount > 0;
                  ...
                  <div className="space-y-3 pt-2 border-t border-gray-200">
                      ...
                      <div className="space-y-2">
                        <div className="relative w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(localProgress.current / localProgress.total) * 100}%`,
                            }}
                          />
                        </div>
                        <div className="text-sm text-gray-700">
                          <span className="font-medium">
                            Processing: {localProgress.current} / {localProgress.total}
                          </span>
                        ...
                        <div className="flex items-center gap-3 text-xs">
                          <span className="text-emerald-700 font-medium">
                            Succeeded: {localProgress.succeeded}
                          </span>
                    ...
                    >
                      {undoingActionId === action.id
                        ? localProgress
                          ? `Undoing... ${localProgress.current}/${localProgress.total}`
                          : 'Undoing...'
                        : remainingCount === 0
                          ? 'All Items Undone'
                          : 'Undo This Action'}
