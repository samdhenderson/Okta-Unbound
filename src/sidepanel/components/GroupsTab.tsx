          ...
          if (age < CACHE_DURATION) {
            const parsedGroups = cached.groups.map((g: any) => ({
              ...
              lastMembershipUpdated: g.lastMembershipUpdated
                ? new Date(g.lastMembershipUpdated)
                : undefined,
    ...
    try {
      ...
      if (fetchPushGroups) {
        ...
        if (allAppIds.size > 0) {
          ...
          const appPromises = Array.from(allAppIds).map(async (appId) => {
            try {
              ...
              if (mappingsResult.length > 0) {
                console.log(
                  `[GroupsTab] App "${appLabel}" (${appId}) has ${mappingsResult.length} push mappings:`,
                  mappingsResult,
                );
              }
              ...
              for (const mapping of mappingsResult) {
                ...
                if (mapping.sourceUserGroupId && mapping.status === 'ACTIVE') {
                  ...
                  if (!existingMappings.some((m) => m.appId === appId)) {
                    ...
                    console.log(
                      `[GroupsTab] Added push mapping: Group ${mapping.sourceUserGroupId} → App "${appLabel}"`,
                    );
                  }
                } else {
                  console.log(
                    `[GroupsTab] Skipping mapping (sourceUserGroupId: ${mapping.sourceUserGroupId}, status: ${mapping.status})`,
                  );
                }
              }
          ...
          console.log(
            '[GroupsTab] Push group mappings by group ID:',
            Object.fromEntries(pushGroupMappings),
          );
      ...
      console.log(
        '[GroupsTab] Groups after applying push mappings:',
        mergedGroups.filter((g) => g.isPushGroup),
      );
              ...
              {searchMode === 'live' ? (
                <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                  ...
                  <span className="text-blue-600">
                    - searching Okta API directly (fast, up to 20 results)
                  </span>
                </div>
              ) : (
                <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-lg text-sm text-green-800">
                  ...
                  <span className="text-green-600">
                    - using loaded groups ({groups.length} total) with powerful filtering
                  </span>
                </div>
              )}
                ...
                filteredGroups.length === 0 && (
                  <div className="text-center py-12 px-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p className="text-gray-700 font-medium mb-1">
                      No groups found matching "{liveSearchQuery}"
                    </p>
                    <p className="text-sm text-gray-500">
                      Try a different search term or load all groups for advanced filtering
                    </p>
                  </div>
                )}
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-600 mb-6">
              Choose whether to include Group Push mappings. This shows which apps OKTA groups are
              pushed to, but is slower.
            </p>
            ...
            <div className="space-y-3 mb-6">
              ...
              >
                <div className="font-medium text-gray-900 group-hover:text-[#007dc1]">
                  Fast Load
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  Load groups without push mappings (Recommended)
                </div>
              </button>
              ...
              >
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Include push group mappings (slower)
                </div>
              </button>
            </div>
