...
const Header: React.FC<HeaderProps> = ({ status }) => {
  ...
  return (
    <header className="relative overflow-hidden gradient-okta-mesh text-white shadow-xl z-50">
      ...
      <div
        className={`absolute top-0 right-0 w-80 h-full bg-gradient-to-l ${statusConfig.bgGradient} transition-all duration-700 ease-out`}
      />
      ...
      <div className="relative px-6 py-5 flex items-center justify-between">
        ...
        <div
          className={`flex items-center gap-3 px-4 py-2.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 ${statusConfig.glowColor} shadow-lg transition-all duration-300`}
        >
          <div className="relative flex items-center justify-center">
            {status === 'connecting' && (
              <span
                className={`absolute w-4 h-4 rounded-full ${statusConfig.dotColor} animate-ping opacity-75`}
              />
            )}
            <span
              className={`relative w-2.5 h-2.5 rounded-full ${statusConfig.dotColor} ring-4 ${statusConfig.ringColor} shadow-lg`}
            />
          </div>
          <span
            className={`text-xs font-semibold ${statusConfig.textColor} tracking-wide`}
            style={{ fontFamily: 'var(--font-primary)' }}
          >
            {statusText}
          </span>
