    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-blue-50/50 rounded-lg border border-blue-200/50">
        <span className="text-sm text-gray-700">
          <span className="font-semibold text-gray-900">{sortedMemberships.length}</span> membership
          {sortedMemberships.length !== 1 ? 's' : ''} requiring review
        </span>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              Memberships older than 90 days should be periodically reviewed to ensure users still
              need access. This is especially important for direct assignments that may have been
              temporary.
            </p>
          </div>
        </div>
            ...
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th
                  ...
                  title="User's Okta account creation date (group membership date not available from Okta API)"
                >
                  Account Created
                  <span
                    className="ml-1 text-amber-500 cursor-help"
                    title="Note: Okta API does not provide group membership dates"
                  >
                    *
                  </span>
                </th>
                <th
                  className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider"
                  title="Days since user was created in Okta"
                >
                  Account Age
                </th>
                <th
                  className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider"
                  title="Heuristic approximation - may not be 100% accurate"
                >
                  Source
                  <span
                    className="ml-1 text-amber-500 cursor-help"
                    title="Approximate - based on rule evaluation heuristics"
                  >
                    *
                  </span>
                </th>
              ...
              {sortedMemberships.map((membership) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                    {membership.daysSinceCreated != null ? (
                      `${membership.daysSinceCreated} days`
                    ) : (
      ...
      <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-2 text-sm text-gray-700">
        ...
        <div className="pt-2 mt-2 border-t border-gray-200 flex items-start gap-2">
          ...
          <span>
            Direct memberships older than 90 days should be reviewed to ensure they are still
        ...
        <div className="pt-2 mt-2 border-t border-gray-200 flex items-start gap-2">
          ...
          <span className="text-gray-600">
            ...
            "Account Created" shows when the user was created in Okta (not when added to this
            group). "Source" is a heuristic approximation based on rule evaluation.
          </span>
        </div>
      </div>
    </div>
