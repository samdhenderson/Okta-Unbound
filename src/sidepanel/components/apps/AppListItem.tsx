...
const AppListItem: React.FC<AppListItemProps> = memo(
  ({ app, selected, onToggleSelect, onEnrich, isEnriching, oktaOrigin }) => {
    ...
    const getAppTypeBadge = (appType: AppSummary['appType']) => {
      const configs = {
        ...
        SAML_1_1: {
          label: 'SAML 1.1',
          gradient: 'from-[#007dc1] to-[#5da3d9]',
          bg: 'bg-blue-50',
          text: 'text-blue-700',
          border: 'border-blue-200',
        },
    ...
    const pushGroupsBadge = getPushGroupsBadge(
      app.pushGroupsEnabled,
      app.pushGroupsCount,
      app.pushGroupsErrors,
    );
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0 cursor-pointer" onClick={toggleExpanded}>
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span
                  className={`px-2.5 py-1 text-xs font-medium rounded-md border ${statusBadge.bg} ${statusBadge.text} ${statusBadge.border}`}
                >
                  {statusBadge.label}
                </span>

                <span
                  className={`px-2.5 py-1 text-xs font-medium rounded-md border ${typeBadge.bg} ${typeBadge.text} ${typeBadge.border}`}
                >
                  {typeBadge.label}
                </span>

                <span
                  className={`px-2.5 py-1 text-xs font-medium rounded-md border ${provisioningBadge.bg} ${provisioningBadge.text} ${provisioningBadge.border}`}
                  title="Provisioning"
                >
                ...
                <span
                  className={`px-2.5 py-1 text-xs font-medium rounded-md border ${pushGroupsBadge.bg} ${pushGroupsBadge.text} ${pushGroupsBadge.border}`}
                  title="Push Groups"
                >
                ...
                {app.appType === 'SAML_2_0' || app.appType === 'SAML_1_1' ? (
                  <span
                    className={`px-2.5 py-1 text-xs font-medium rounded-md border ${certBadge.bg} ${certBadge.text} ${certBadge.border}`}
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
              ...
              {app.certExpiresAt && (
                <div>
                  ...
                  <span className="ml-2 text-gray-900">
                    {new Date(app.certExpiresAt).toLocaleDateString()}
                  </span>
                </div>
              )}
            </div>
