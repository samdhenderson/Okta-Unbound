...
const SecurityTab: React.FC<SecurityTabProps> = ({ groupId, groupName, targetTabId }) => {
    ...
    try {
      ...
      const userDetailsMap = await batchGetUserDetails(userIds, (current, total) => {
        ...
        setScanProgress({
          current: progress,
          total: 100,
          message: `Loading user details ${current}/${total}...`,
        });
      });
  ...
  const handleExportFullReport = () => {
    if (scanData && scanData.posture) {
      exportSecurityReportToCSV(
        scanData.posture,
        scanData.orphanedAccounts,
        scanData.staleMemberships,
      );
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-600 max-w-md">
              Please navigate to an Okta group page to run security analysis.
            </p>
          </div>
        </div>
      </div>
  ...
  const criticalCount =
    scanData?.posture?.findings.filter((f) => f.severity === 'critical').length || 0;
  ...
  return (
    <div className="tab-content active" style={{ fontFamily: 'var(--font-primary)', padding: 0 }}>
      <PageHeader
        ...
        icon="shield"
        badge={
          criticalCount > 0 ? { text: `${criticalCount} Critical`, variant: 'error' } : undefined
        }
        actions={
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-600 mt-1">
                Includes app assignments and group memberships count for each user. This requires 2
                additional API calls per orphaned user and may significantly slow down the scan for
                large groups.
              </p>
              {fetchUserMetadata && (
                <div className="mt-2 flex items-start gap-2 p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
                  ...
                  <span>
                    Warning: This will make the scan significantly slower. For a group with 100
                    orphaned users, expect ~200 additional API calls and 30-60 seconds of extra scan
                    time.
                  </span>
                </div>
              )}
                ...
                >
                  <div
                    style={{
                      fontSize: '32px',
                      fontWeight: 'bold',
                      color: getScoreColor(scanData.posture.overallScore),
                    }}
                  >
        ...
        {!scanData && !isScanning && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#007dc1]/10 to-[#3d9dd9]/10 flex items-center justify-center mb-6 shadow-sm">
              <svg
                className="w-10 h-10 text-[#007dc1]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-gray-600 mb-6 max-w-md">
              Click "Run Security Scan" to analyze this group for security risks.
            </p>
        ...
        {scanData && (
          <>
            ...
            {activeSubTab === 'stale' && (
              <StaleMembershipsList
                memberships={scanData.staleMemberships}
                groupName={groupName || 'Unknown Group'}
              />
            )}
              ...
              <div className="findings-view">
                {scanData.posture.findings.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-gray-600 max-w-md">
                      This group looks secure! No issues detected.
                    </p>
                  </div>
                ) : (
                  <div
                      ...
                      {scanData.posture.recommendations.map((reco, index) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              className={`priority-badge priority-${reco.priority}`}
                              style={{
                                fontSize: '12px',
                                padding: '4px 8px',
                                borderRadius: '4px',
      ...
      {showRemoveModal && (
        <ConfirmationModal
          ...
          message={
            <div>
              <p>
                You are about to remove {usersToRemove.length} user(s) from {groupName}.
              </p>
              {scanData && (
                <div style={{ marginTop: '12px' }}>
                  ...
                  <ul style={{ marginTop: '8px' }}>
                    <li>
                      Critical:{' '}
                      {
                        scanData.orphanedAccounts.filter(
                          (a) => usersToRemove.includes(a.userId) && a.riskLevel === 'critical',
                        ).length
                      }
                    </li>
                    <li>
                      ...
                      {
                        scanData.orphanedAccounts.filter(
                          (a) => usersToRemove.includes(a.userId) && a.riskLevel === 'high',
                        ).length
                      }
                    </li>
                    <li>
                      ...
                      {
                        scanData.orphanedAccounts.filter(
                          (a) => usersToRemove.includes(a.userId) && a.riskLevel === 'medium',
                        ).length
                      }
                    </li>
                  </ul>
              ...
              <p style={{ marginTop: '12px', color: '#dc3545' }}>
                <strong>Warning:</strong> This action cannot be undone via the extension. You will
                need to manually re-add these users if needed.
              </p>
            </div>
          }
