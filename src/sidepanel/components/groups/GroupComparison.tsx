...
const GroupComparison: React.FC<GroupComparisonProps> = ({ selectedGroupIds, onCompare }) => {
  ...
  const renderVennDiagram = () => {
    if (!comparison || comparison.groupData.length < 2 || comparison.groupData.length > 3)
      return null;
      ...
      <div className="bg-gradient-to-br from-gray-50 to-white rounded-lg border border-gray-200 p-8 shadow-sm">
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          <div className="flex flex-col items-center p-3 bg-gradient-to-br from-blue-50 to-white rounded-lg border border-blue-200">
            <div className="text-2xl font-bold" style={{ color: colors.group1.stroke }}>
              {overlap.uniqueToGroup1}
            </div>
            <div className="text-xs text-gray-600 mt-1 text-center">
              Only in {group1.name.substring(0, 20)}
              {group1.name.length > 20 ? '...' : ''}
            </div>
          </div>
          ...
          <div className="flex flex-col items-center p-3 bg-gradient-to-br from-purple-50 to-white rounded-lg border border-purple-200">
            <div className="text-2xl font-bold" style={{ color: colors.group2.stroke }}>
              {overlap.uniqueToGroup2}
            </div>
            <div className="text-xs text-gray-600 mt-1 text-center">
              Only in {group2.name.substring(0, 20)}
              {group2.name.length > 20 ? '...' : ''}
            </div>
          </div>
        </div>
      </div>
  ...
  const renderThreeGroupVenn = () => {
    ...
    const intersection123 = new Set(
      [...members1].filter((x) => members2.has(x) && members3.has(x)),
    );
        ...
        <svg viewBox="0 0 500 350" className="w-full h-auto" style={{ maxHeight: '380px' }}>
          ...
          <g filter="url(#shadow3)">
            ...
            <circle
              ...
              cy="225"
              r="90"
              fill={colors.group3.fill}
              stroke={colors.group3.stroke}
              strokeWidth="2.5"
            />
          </g>
                ...
                <tbody className="divide-y divide-gray-200">
                  {comparison.overlaps?.map((overlap: any, idx: number) => (
                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors duration-150">
                      ...
                      <td className="px-4 py-3 text-sm font-semibold text-[#007dc1]">
                        {overlap.sharedUsers}
                      </td>
