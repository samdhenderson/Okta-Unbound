...
const BulkOperations: React.FC<BulkOperationsProps> = ({ selectedGroupIds, onExecute }) => {
                ...
                {result.errors && result.errors.length > 0 && (
                  <div className="mt-3 space-y-1.5">
                    {result.errors.map((error, idx) => (
                      <div
                        key={idx}
                        className="text-sm text-red-700 bg-red-50 p-2 rounded border-l-2 border-red-400"
                      >
                        {error}
                      </div>
                    ))}
