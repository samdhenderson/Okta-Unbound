...
const OperationsTab: React.FC<OperationsTabProps> = ({ groupId, groupName, targetTabId }) => {
  ...
  const [customFilterStatuses, setCustomFilterStatuses] = useState<Set<UserStatus>>(
    new Set(['DEPROVISIONED']),
  );
          ...
          <div className="status-multi-select">
            ...
            {customFilterStatuses.size > 0 && (
              <div className="selected-count">
                {customFilterStatuses.size} status{customFilterStatuses.size !== 1 ? 'es' : ''}{' '}
                selected
              </div>
            )}
          </div>
