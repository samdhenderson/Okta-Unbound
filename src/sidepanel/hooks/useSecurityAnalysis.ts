    ...
    async (
      ...
      for (let i = 0; i < groupMembers.length; i++) {
          ...
          if (fetchUserMetadata && getUserAppAssignments && getUserGroupMemberships) {
            try {
              updateProgress(
                i + 1,
                groupMembers.length,
                `Fetching metadata for user ${i + 1}/${groupMembers.length}...`,
              );
              ...
              console.error(
                `[useSecurityAnalysis] Failed to fetch metadata for user ${member.id}:`,
                error,
              );
            }
          }
        ...
        updateProgress(
          i + 1,
          groupMembers.length,
          `Analyzed ${i + 1} of ${groupMembers.length} users`,
        );
      }
    ...
    async (
      ...
      for (let i = 0; i < groupMembers.length; i++) {
        ...
        updateProgress(
          i + 1,
          groupMembers.length,
          `Analyzed ${i + 1} of ${groupMembers.length} memberships`,
        );
      }
    ...
    (
      ...
      if (staleMemberships.length > 0) {
        ...
        recommendations.push({
          ...
          description:
            'Memberships older than 90 days should be reviewed to ensure they are still necessary.',
          priority: 'medium',
          actionable: true,
      ...
      if (cache && cache.groupId === groupId) {
        ...
        if (age < CACHE_TTL) {
          ...
          const staleMemberships = cache.staleMemberships.map((membership) => ({
            ...membership,
            userCreatedDate: membership.userCreatedDate
              ? new Date(membership.userCreatedDate)
              : null,
          }));
