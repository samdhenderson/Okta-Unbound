...
export function createGroupCleanupOperations(
    ...
    try {
      ...
      while (nextUrl) {
        ...
        coreApi.callbacks.onProgress?.(
          0,
          100,
          `Fetching members (page ${pageCount})...`,
          apiCallsMade,
        );
    ...
    try {
      ...
      for (let i = 0; i < inactiveUsers.length; i++) {
        ...
        if (result.success) {
          ...
          coreApi.callbacks.onResult?.(
            `Removed: ${user.profile.login} (${user.status})`,
            'success',
          );
        } else {
          failed++;
      ...
      coreApi.callbacks.onResult?.(
        `Smart Cleanup complete: ${removed} removed, ${failed} failed`,
        'success',
      );
    } catch (error) {
      const errorMsg = `Error: ${error instanceof Error ? error.message : 'Unknown error'}`;
  ...
  const customFilter = async (
    ...
    try {
      ...
      coreApi.callbacks.onResult?.(
        `Found ${filteredUsers.length} users with status ${targetStatus}`,
        'warning',
      );
  ...
  const customFilterMultiple = async (
    groupId: string,
    ...
    try {
      ...
      coreApi.callbacks.onResult?.(
        `Found ${filteredUsers.length} users matching statuses: ${statusLabel}`,
        'warning',
      );
