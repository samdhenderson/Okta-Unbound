import{j as n}from"./iframe-mmN7AxbW.js";import{u as a,M as r,c as i}from"./blocks-6hb-cuWT.js";import"./preload-helper-PPVm8Dsz.js";const o=`# Shared utilities
...
Okta Unbound Internals / shared/utils/apiPath / isNormalizedPath

# Function: isNormalizedPath()

> **isNormalizedPath**(\`endpoint\`, \`origin?\`): \`boolean\`

Defined in: [src/shared/utils/apiPath.ts:120](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/apiPath.ts#L120)

Whether a path's own text is what the URL parser makes of it.

The second half of the same-origin guard. \`isSameOriginPath\` proves the request
cannot leave the org; this proves it cannot address a different resource *within*
the org than the one it appears to name — \`/api/v1/../admin/users\` is same-origin
and reads as an API call, and resolves to \`/admin/users\`.

Only the path is normalised. The query string is left exactly as assembled,
because Okta's \`search\` and \`filter\` expressions carry spaces and quotes that a
round-trip comparison would reject and \`fetch\` encodes correctly anyway.
...
A same-origin path, query string included.

### origin?

\`string\` = \`window.location.origin\`

Base to resolve against. Defaults to the current origin.

## Returns

\`boolean\`

\`true\` when the path survives a parse unchanged.
...
Okta Unbound Internals / shared/utils/apiPath / isSafePathSegment

# Function: isSafePathSegment()

> **isSafePathSegment**(\`value\`): \`boolean\`

Defined in: [src/shared/utils/apiPath.ts:46](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/apiPath.ts#L46)

Whether a value may be substituted into a path as one segment.
...
The candidate, as the reader typed or picked it.

## Returns

\`boolean\`

\`true\` when the value cannot change the shape of the path.

## Example

\`\`\`ts
isSafePathSegment('00uFAKE000000000000');   // true
isSafePathSegment('me');                     // true — and a real Okta path
isSafePathSegment('../../admin/users');      // false
\`\`\`
...
Okta Unbound Internals / shared/utils/apiPath / substitutePathHoles

# Function: substitutePathHoles()

> **substitutePathHoles**(\`template\`, \`values\`): \`SubstitutionResult\`

Defined in: [src/shared/utils/apiPath.ts:81](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/apiPath.ts#L81)

Fill every \`{token}\` in a template with the value given for it.

Each accepted value is \`encodeURIComponent\`-ed, so a login containing \`@\` or a
\`+\` survives as one segment instead of being read as structure.
...
A path with \`{token}\` holes.

### values

\`Readonly\`\\<\`Record\`\\<\`string\`, \`string\`\\>\\>

Value per token. A missing token is an error, not an empty
  segment — a path with a hole silently removed addresses a different resource.

## Returns

\`SubstitutionResult\`

The filled path, or the first token that refused and why.

## Example

\`\`\`ts
substitutePathHoles('/api/v1/users/{userId}', { userId: 'ada@example.com' });
// { ok: true, path: '/api/v1/users/ada%40example.com' }
\`\`\`
...
Okta Unbound Internals / shared/utils/apiPath / SubstitutionError

# Type Alias: SubstitutionError

> **SubstitutionError** = \\{ \`reason\`: \`"unfilled"\`; \`token\`: \`string\`; \\} \\| \\{ \`reason\`: \`"unsafe"\`; \`token\`: \`string\`; \\}

Defined in: [src/shared/utils/apiPath.ts:56](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/apiPath.ts#L56)

Why a substitution was refused, as a reason code rather than a sentence.
...
Okta Unbound Internals / shared/utils/apiPath / SubstitutionResult

# Type Alias: SubstitutionResult

> **SubstitutionResult** = \\{ \`ok\`: \`true\`; \`path\`: \`string\`; \\} \\| \\{ \`ok\`: \`false\`; \`error\`: \`SubstitutionError\`; \\}

Defined in: [src/shared/utils/apiPath.ts:61](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/apiPath.ts#L61)

The outcome of filling a path template.
...
Defined in: [src/shared/utils/membershipAnalysis.ts:312](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L312)
...
Okta Unbound Internals / shared/utils/membershipAnalysis / exclusionRouteOf

# Function: exclusionRouteOf()

> **exclusionRouteOf**(\`rule\`, \`userId\`, \`groups\`): \`ExclusionRoute\`

Defined in: [src/shared/utils/membershipAnalysis.ts:187](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L187)

isUserExcluded with the route named, for a surface that states *why*
a rule passes this user over. The user route is checked first: it is stated
on the rule itself, so it is the stronger fact when both apply.
...
The user's complete group list, or \`undefined\`.

## Returns

\`ExclusionRoute\`

The route, or \`'none'\` when the rule is not established to exclude them.
...
Defined in: [src/shared/utils/membershipAnalysis.ts:258](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L258)
...
Defined in: [src/shared/utils/membershipAnalysis.ts:268](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L268)
...
Defined in: [src/shared/utils/membershipAnalysis.ts:279](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L279)
...
Okta Unbound Internals / shared/utils/membershipAnalysis / ExclusionRoute

# Type Alias: ExclusionRoute

> **ExclusionRoute** = \`"none"\` \\| \`"user"\` \\| \`"group"\`

Defined in: [src/shared/utils/membershipAnalysis.ts:176](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/membershipAnalysis.ts#L176)

Which route a rule's exclusion took. \`'none'\` is a fact only when \`groups\` was
the user's complete list — with no context the group route is unanswerable
and reads as \`'none'\`, exactly as isUserExcluded reads it as \`false\`.
...
Okta Unbound Internals / shared/utils/password / generatePassword

# Function: generatePassword()

> **generatePassword**(\`length?\`): \`string\`

Defined in: [src/shared/utils/password.ts:76](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/password.ts#L76)
...
\`number\` = \`DEFAULT_GENERATED_LENGTH\`

How many characters. Clamped up to MIN\\_GENERATED\\_LENGTH.

## Returns

\`string\`

The generated value. Never logged, never stored by this module.

## Remarks

An org's password policy is still the authority — a generated value
can be refused, and Okta's rejection message names the rule it broke.

## Example

\`\`\`ts
const value = generatePassword();       // 20 characters, all four classes
\`\`\`
...
Okta Unbound Internals / shared/utils/password / DEFAULT\\_GENERATED\\_LENGTH

# Variable: DEFAULT\\_GENERATED\\_LENGTH

> \`const\` **DEFAULT\\_GENERATED\\_LENGTH**: \`20\` = \`20\`

Defined in: [src/shared/utils/password.ts:44](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/password.ts#L44)

What the generate affordance produces when nobody says otherwise.
...
Okta Unbound Internals / shared/utils/password / MIN\\_GENERATED\\_LENGTH

# Variable: MIN\\_GENERATED\\_LENGTH

> \`const\` **MIN\\_GENERATED\\_LENGTH**: \`12\` = \`12\`

Defined in: [src/shared/utils/password.ts:41](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/password.ts#L41)

Shortest value this will produce: one character per class, plus room to spare.
...
Defined in: [src/shared/utils/regexQuery.ts:25](https://github.com/samdhenderson/Okta-Unbound-Dev/blob/main/src/shared/utils/regexQuery.ts#L25)
...
A non-empty display string.`;function s(e){return n.jsxs(n.Fragment,{children:[`
`,n.jsx(r,{title:"Internals/Shared utilities"}),`
`,n.jsx(i,{children:o})]})}function h(e={}){const{wrapper:t}={...a(),...e.components};return t?n.jsx(t,{...e,children:n.jsx(s,{...e})}):s()}export{h as default};
