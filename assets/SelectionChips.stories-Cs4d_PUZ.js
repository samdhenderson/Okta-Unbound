import{al as v,j as y,r as E}from"./iframe-mmN7AxbW.js";import{m as t}from"./fixtures-CsAiPaTu.js";import"./preload-helper-PPVm8Dsz.js";const{expect:h,fn:d,userEvent:f,within:b}=__STORYBOOK_MODULE_TEST__,g=e=>e,A={title:"Shared/SelectionChips",component:v,tags:["autodocs"],parameters:{layout:"centered",docs:{description:{component:"Renders a set of selected items as removable chips — the display half of a multi-select.\n\nGeneric over the item type `T`; `getKey`/`getLabel` project each item. Shows `emptyMessage` when empty, and a “Clear all” link only when more than one item is selected."}}},argTypes:{items:{description:"Selected items to render as chips."},getKey:{description:"Stable React key for an item."},getLabel:{description:"Visible chip label for an item."},onRemove:{description:"Called to remove a single item (its × button)."},onClearAll:{description:"Optional “Clear all” handler; the link shows only when >1 item is selected."},emptyMessage:{description:"Placeholder text shown when there are no items."},className:{description:"Extra classes merged onto the outer container."}},args:{items:[],getKey:e=>g(e).id,getLabel:e=>{const s=g(e);return`${s.profile.firstName} ${s.profile.lastName}`},onRemove:d()}},a={},n={args:{emptyMessage:"No users assigned"}},o={args:{items:[t[0]]}},i={args:{items:t.slice(0,3)}},c={args:{items:t.slice(0,5),onClearAll:d()}},l={args:{items:[t[0]],onClearAll:d()}},m={args:{items:t.slice(0,10),onClearAll:d()}},p={args:{items:t.slice(0,3),onClearAll:d()},render:e=>{const s=()=>{const[u,r]=E.useState(e.items);return y.jsx(v,{...e,items:u,onRemove:S=>r(C=>C.filter(w=>w!==S)),onClearAll:()=>r([])})};return y.jsx(s,{})},play:async({canvasElement:e})=>{const s=b(e),u=g(t[0]),r=`${u.profile.firstName} ${u.profile.lastName}`;await f.click(s.getByRole("button",{name:`Remove ${r}`})),await h(s.queryByText(r)).not.toBeInTheDocument(),await f.click(s.getByRole("button",{name:"Clear all"})),await h(s.getByText("No items selected")).toBeInTheDocument()}};a.parameters={...a.parameters,docs:{...a.parameters?.docs,source:{originalSource:"{}",...a.parameters?.docs?.source},description:{story:"Empty state.",...a.parameters?.docs?.description}}};n.parameters={...n.parameters,docs:{...n.parameters?.docs,source:{originalSource:`{
  args: {
